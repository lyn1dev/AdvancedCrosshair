...
public class InGameHudMixin {
    ...
    private void advancedcrosshair$drawSprite(GuiGraphicsExtractor graphics, RenderPipeline pipeline,
        ...
        if (CROSSHAIR_SPRITE_PATH.equals(path)) {
            ...
            if (tint == NO_TINT) {
                graphics.blitSprite(pipeline, sprite, drawX, drawY, scaledWidth, scaledHeight);
            } else {
                ...
                graphics.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, drawX, drawY, scaledWidth, scaledHeight, tint);
            }
            return;
        }

        if (path.startsWith(ATTACK_INDICATOR_PREFIX)) {
            graphics.blitSprite(pipeline, sprite, x, y + advancedcrosshair$indicatorOffset, width, height);
            return;
        }
    ...
    private void advancedcrosshair$drawSpriteRegion(GuiGraphicsExtractor graphics, RenderPipeline pipeline,
        ...
        int offset = config.modEnabled && sprite.getPath().startsWith(ATTACK_INDICATOR_PREFIX)
                ...
                : 0;
        graphics.blitSprite(pipeline, sprite, textureWidth, textureHeight, u, v, x, y + offset, width, height);
    }

    /**
     * Vanilla skips the flat crosshair entirely while the F3 debug screen draws
     * its three-dimensional one. Reporting that entry as disabled puts the normal
    ...
    private boolean advancedcrosshair$debugCrosshairEnabled(DebugScreenEntryList entries, Identifier entry) {
        boolean enabled = entries.isCurrentlyEnabled(entry);
        if (!enabled) {
            return false;
        }
        AdvancedCrosshairConfig config = AdvancedCrosshairConfig.get();
